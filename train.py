...
for epoch in range(1, epochs + 1):
    ...
    for en_in, de_in, de_out, path in train_loader:
        ...
        # Debug shapes to check for errors
        print("de_out_pred shape:", de_out_pred.shape)
        print("de_out shape:", de_out.shape)
        print("path_pred shape:", path_pred.shape)
        print("path shape:", path.shape)

        # Ensure de_out and path are class indices
        assert de_out.max() < output_size, f"Invalid class index {de_out.max()} for target de_out"
        assert path.max() < output_size, f"Invalid class index {path.max()} for target path"
